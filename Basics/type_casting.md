...
Primitive type casting deals with **converting one primitive data type into another** (like `int`, `float`, `double`, `byte`, etc.). There are two main kinds of primitive type casting — **Widening (Implicit)** and **Narrowing (Explicit)**.

a) **Widening Type Casting (Implicit Casting) -**

Widening happens **automatically** when a smaller data type is converted into a larger data type. It is also called **upcasting** because data moves from a lower range to a higher range without data loss.
...
Reference type casting applies to **objects** and deals with **inheritance**. It allows an object of one class type to be treated as another type within the same inheritance hierarchy.

There are two kinds of reference casting — **Upcasting** and **Downcasting**.

a) **Upcasting -**

Upcasting means **converting a subclass object into a superclass reference**. It is done automatically by Java because every subclass object “is a kind of” its superclass. Upcasting is used when you want to generalize code or call overridden methods.
